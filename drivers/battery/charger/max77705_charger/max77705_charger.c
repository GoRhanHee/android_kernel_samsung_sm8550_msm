...
static void max77705_set_sysuvlo(struct max77705_charger_data *charger, int enable)
{
	u8 reg_data;

	max77705_read_reg(charger->pmic_i2c, MAX77705_PMIC_REG_SYSTEM_INT_MASK, &reg_data);

	reg_data = enable ? reg_data & 0xEF : reg_data | 0x10;
	max77705_write_reg(charger->pmic_i2c, MAX77705_PMIC_REG_SYSTEM_INT_MASK, reg_data);
...
static void max77705_chg_monitor_work(struct max77705_charger_data *charger)
{
	...
	reg_b2sovrc = (reg_b2sovrc & CHG_CNFG_05_REG_B2SOVRC_MASK) >> CHG_CNFG_05_REG_B2SOVRC_SHIFT;
	...
	if (reg_b2sovrc == CHG_CNFG_05_REG_B2SOVRC_RST) {
		pr_info("%s: b2sovrc_rst - charger initialize\n", __func__);
		max77705_charger_initialize(charger);
	}
					...
					const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (is_nocharge_type(charger->cable_type)) {
			...
			if (charger->pdata->enable_sysuvlo_irq)
				max77705_set_sysuvlo(charger, 1);
...
static irqreturn_t max77705_sysuvlo_irq(int irq, void *data)
{
	struct max77705_charger_data *charger = data;
	//union power_supply_propval value = {0, };

	pr_info("%s\n", __func__);
	__pm_wakeup_event(charger->sysuvlo_ws, jiffies_to_msecs(HZ * 5));

	//psy_do_property("battery", set, POWER_SUPPLY_EXT_PROP_SYSUVLO, value);

	max77705_set_sysuvlo(charger, 0);
	return IRQ_HANDLED;
}
...
static int max77705_charger_parse_dt(struct max77705_charger_data *charger)
{
	...
	if (!np) {
		...
		pdata->enable_sysovlo_irq = of_property_read_bool(np, "charger,enable_sysovlo_irq");
		pdata->enable_sysuvlo_irq = of_property_read_bool(np, "charger,enable_sysuvlo_irq");
...
static int max77705_charger_probe(struct platform_device *pdev)
{
	...
	if (charger->pdata->enable_sysovlo_irq) {
		...
		/* Enable BIAS */
		max77705_update_reg(max77705->i2c, MAX77705_PMIC_REG_MAINCTRL1, 0x80, 0x80);
		/* set IRQ thread */
		charger->irq_sysovlo = pdata->irq_base + MAX77705_SYSTEM_IRQ_SYSOVLO_INT;
		ret = request_threaded_irq(charger->irq_sysovlo, NULL, max77705_sysovlo_irq, 0, "sysovlo-irq", charger);
		if (ret < 0)
	...
	if (charger->pdata->enable_sysuvlo_irq) {
		charger->sysuvlo_ws = wakeup_source_register(&pdev->dev, "max77705-sysuvlo");
		/* Enable BIAS */
		max77705_update_reg(max77705->i2c, MAX77705_PMIC_REG_MAINCTRL1, 0x80, 0x80);
		/* set IRQ thread */
		charger->irq_sysuvlo = pdata->irq_base + MAX77705_SYSTEM_IRQ_SYSUVLO_INT;
		ret = request_threaded_irq(charger->irq_sysuvlo, NULL, max77705_sysuvlo_irq, 0, "sysuvlo-irq", charger);
		if (ret < 0)
			pr_err("%s: fail to request sysuvlo IRQ: %d: %d\n", __func__, charger->irq_sysuvlo, ret);
		enable_irq_wake(charger->irq_sysuvlo);
	}
...
err_power_supply_register:
	...
	wakeup_source_unregister(charger->sysuvlo_ws);
...
static int max77705_charger_remove(struct platform_device *pdev)
{
	...
	if (charger->irq_sysuvlo)
		free_irq(charger->irq_sysuvlo, charger);
#if defined(CONFIG_USE_POGO)
	if (charger->irq_wcin)
