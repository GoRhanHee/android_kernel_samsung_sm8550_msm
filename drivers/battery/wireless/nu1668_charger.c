...
// need to define TX_ID_CHECK_CNT.
...
static void mfc_cma_cmb_onoff(struct mfc_charger_data *charger, bool cma_on, bool cmb_on)
{
	...
	data = ((cma_on) ? 0xC0 : 0x00) | ((cmb_on) ? 0x30 : 0x00);//cma1_on:bit7,cma2_on:bit6; cmb1_on:bit5,cma2_on:bit4;
	pr_info("%s: val(0x%x)\n", __func__, data);
						...
						const struct i2c_device_id *id)
{
	...
	pr_info("%s: MFC NU1668 Charger Driver Loaded\n", __func__);
