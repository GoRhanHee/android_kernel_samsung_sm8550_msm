...
static int max77705_write_and_verify_word(struct max77705_fuelgauge_data *fuelgauge, u8 reg, u16 val)
{
	u16 r_data;
	u8 retry_cnt = 2;

retry_write:
	max77705_write_word(fuelgauge->i2c, reg, val);
	msleep(3);
	r_data = max77705_read_word(fuelgauge->i2c, reg);

	if (r_data != val) {
		pr_err("%s: failed w_data: 0x%x, r_data: 0x%x\n", __func__, val, r_data);
		if (retry_cnt--)
			goto retry_write;
		else
			return -1;
...
static void max77705_fg_test_read(struct max77705_fuelgauge_data *fuelgauge)
{
	u8 i = 0, reg_list_size = 0;
	u16 data = 0;
	static u8 reg_list[] = {
		0x00, 0x05, 0x06, 0x08, 0x09, 0x0A, 0x0B, 0x0D, 0x0E, 0x0F, 0x10, 0x35, 0x12,
		0x13, 0x17, 0x18, 0x19, 0x1D, 0x1E, 0x1F, 0x22, 0x23, 0x24, 0x25, 0x28, 0x29,
		0x2A, 0x2B, 0x2C, 0x2D, 0x2E, 0x2F, 0x32, 0x36, 0x38, 0x39, 0x3A, 0x3C, 0x42,
		0x45, 0x46, 0x48, 0x49, 0xBA, 0xBB, 0xB6, 0xB7, 0xDB, 0xFB, 0xFF
	};

	reg_list_size = ARRAY_SIZE(reg_list);
	for (i = 0; i < reg_list_size; i++) {
		data = max77705_read_word(fuelgauge->i2c, reg_list[i]);
		pr_info("%s: reg_addr = 0x%02x read_data = 0x%04xh\n", __func__, reg_list[i], data);
	}
}

static int max77705_update_MiscCFG(struct max77705_fuelgauge_data *fuelgauge, u16 val)
{
	u16 read_val;

	read_val = max77705_read_word(fuelgauge->i2c, MISCCFG_REG);
	read_val &= 0x0004; // get only Vex(BIT2)

	val = read_val | val;
	pr_info("%s: MiscCFG val (0x%04x)\n", __func__, val);
	return max77705_write_and_verify_word(fuelgauge, MISCCFG_REG, val);// write misccfg
}

static int max77705_fg_initialization(struct max77705_fuelgauge_data *fuelgauge)
{
	//parse cellular data
	u16 read_val, Config2value; //, status;
	u16 r_data0[16], r_data1[16];
	u32 i = 0, count = 0;
	//u8 retry = max77705_RETRY_COUNT;
	int fail;
	int reinit_status = 0;
	...
	/* to get stable ocv without charging current */
	//max77705_check_fuelgauge_powered();
	...
	do {
		...
		/* Check POR */
	//		status = max77705_read_word(fuelgauge->i2c, 0x00);
		/* Read Status */
		read_val = max77705_read_word(fuelgauge->i2c, STATUS_REG);
		/* Write and Verify Status with POR bit Cleared */
		if (read_val >= 0)
			reinit_status += max77705_write_and_verify_word(fuelgauge, STATUS_REG,
				(read_val & 0xFFFD));

		while ((max77705_read_word(fuelgauge->i2c, 0x3D) & 1) && count < 50) {
			msleep(10);
			count++;
		}

		/* 3. Unlock model access */
		reinit_status += max77705_write_and_verify_word(fuelgauge, 0x62, 0x0059);
		reinit_status += max77705_write_and_verify_word(fuelgauge, 0x63, 0x00C4);

		/* 4. Write the custom model */
		for (i = 0x80; i <= 0x8F; i++)
			reinit_status += max77705_write_and_verify_word(fuelgauge, i, fuelgauge->battery_data->cell_character[0][i-0x80]);
		for (i = 0x90; i <= 0x9F; i++)
			reinit_status += max77705_write_and_verify_word(fuelgauge, i, fuelgauge->battery_data->cell_character[1][i-0x90]);

		/* Read the model */
		for (i = 0x80; i <= 0x8F; i++)
			r_data0[i-0x80] = max77705_read_word(fuelgauge->i2c, i);
		for (i = 0x90; i <= 0x9F; i++)
			r_data1[i-0x90] = max77705_read_word(fuelgauge->i2c, i);

		/* Verify the model */
		for (i = 0; i < 16; i++) {
			if ((fuelgauge->battery_data->cell_character[0][i] != r_data0[i])
				|| (fuelgauge->battery_data->cell_character[1][i] != r_data1[i])) {
				pr_info("%s: step4. verify failed!!\n", __func__);
				fail = 1;
				break;
			}
		}
		...
		/* 5. Lock model access */
		reinit_status += max77705_write_and_verify_word(fuelgauge, 0x62, 0x0000);
		reinit_status += max77705_write_and_verify_word(fuelgauge, 0x63, 0x0000);
		...
		/* 6. Verify the model access is locked */
		for (i = 0x80; i <= 0x8F; i++)
			r_data0[i-0x80] = max77705_read_word(fuelgauge->i2c, i);
		for (i = 0x90; i <= 0x9F; i++)
			r_data1[i-0x90] = max77705_read_word(fuelgauge->i2c, i);
		...
		for (i = 0; i < 16; i++) {
			if (((r_data0[i] != 0) && (r_data0[i] != 0xffff)) ||
				((r_data1[i] != 0) && (r_data1[i] != 0xffff))) {
				pr_info("%s : Lock model access failed, Rewrite it now!!\n", __func__);
				fail = 2;
				break;
			}
		}
		...
		/* 7. Write custom parameters (all parameters from MAXIM) */
		reinit_status += max77705_write_and_verify_word(fuelgauge, REMCAP_REP_REG, 0x00);
		reinit_status += max77705_write_and_verify_word(fuelgauge, IAVGEMPTY_REG,
			fuelgauge->battery_data->iavg_empty);
		reinit_status += max77705_write_and_verify_word(fuelgauge, RELAXCFG_REG,
			fuelgauge->battery_data->relaxcfg);
		/* unlock extra config registers for write access */
		reinit_status += max77705_write_and_verify_word(fuelgauge, 0x60, 0x0080);
		read_val = max77705_read_word(fuelgauge->i2c, VFSOC_REG);
		if (read_val >= 0)
			reinit_status += max77705_write_and_verify_word(fuelgauge, VFSOC_REG,
				read_val);
		reinit_status += max77705_write_and_verify_word(fuelgauge, LEARN_CFG_REG,
			fuelgauge->battery_data->learnCFG);
		reinit_status += max77705_write_and_verify_word(fuelgauge, CONFIG_REG,
			fuelgauge->battery_data->config);
		reinit_status += max77705_write_and_verify_word(fuelgauge, CONFIG2_REG,
			fuelgauge->battery_data->config2);
		reinit_status += max77705_write_and_verify_word(fuelgauge, FULLSOCTHR_REG,
			fuelgauge->battery_data->fullsocthr);
		reinit_status += max77705_write_and_verify_word(fuelgauge, FULLCAP_REP_REG,
			fuelgauge->battery_data->fullcaprep);
		reinit_status += max77705_write_and_verify_word(fuelgauge, DESIGNCAP_REG,
			fuelgauge->battery_data->Capacity);
		reinit_status += max77705_write_and_verify_word(fuelgauge, DPACC_REG,
			fg_reset_data->dPacc);
		reinit_status += max77705_write_and_verify_word(fuelgauge, DQACC_REG,
			fg_reset_data->dQacc);
		reinit_status += max77705_write_and_verify_word(fuelgauge, FULLCAP_NOM_REG,
			fg_reset_data->fullcapnom);
		reinit_status += max77705_update_MiscCFG(fuelgauge, fuelgauge->battery_data->misccfg);
		reinit_status += max77705_write_and_verify_word(fuelgauge, VEMPTY_REG,
			fuelgauge->battery_data->V_empty);
		reinit_status += max77705_write_and_verify_word(fuelgauge, QRTABLE00_REG,
			fuelgauge->q_res_table[0]);
		reinit_status += max77705_write_and_verify_word(fuelgauge, QRTABLE10_REG,
			fuelgauge->q_res_table[1]);
		reinit_status += max77705_write_and_verify_word(fuelgauge, QRTABLE20_REG,
			fuelgauge->q_res_table[2]);
		reinit_status += max77705_write_and_verify_word(fuelgauge, QRTABLE30_REG,
			fuelgauge->q_res_table[3]);
		reinit_status += max77705_write_and_verify_word(fuelgauge, RCOMP_REG,
			fg_reset_data->rcomp0);
		reinit_status += max77705_write_and_verify_word(fuelgauge, TEMPCO_REG,
			fg_reset_data->tempco);
		reinit_status += max77705_write_and_verify_word(fuelgauge, ICHGTERM_REG,
			fuelgauge->battery_data->ichgterm);
		reinit_status += max77705_write_and_verify_word(fuelgauge, TGAIN_REG,
			fuelgauge->battery_data->tgain);
		reinit_status += max77705_write_and_verify_word(fuelgauge, TOFF_REG,
			fuelgauge->battery_data->toff);
		reinit_status += max77705_write_and_verify_word(fuelgauge, TEMPNOM_REG,
			fuelgauge->battery_data->tempNom);
		reinit_status += max77705_write_and_verify_word(fuelgauge, TEMPLIM_REG,
			fuelgauge->battery_data->tempLim);
		reinit_status += max77705_write_and_verify_word(fuelgauge, CGAIN_REG,
			fuelgauge->battery_data->cgain_origin);
		reinit_status += max77705_write_and_verify_word(fuelgauge, COFFSET_REG,
			fuelgauge->battery_data->coff_origin);

		/* M5 New Parameters */
		reinit_status += max77705_write_and_verify_word(fuelgauge, 0x60, 0x0080);
		reinit_status += max77705_write_and_verify_word(fuelgauge, 0x04, fuelgauge->battery_data->atRate);
		reinit_status += max77705_write_and_verify_word(fuelgauge, CVMIXCAP_REG,
			fuelgauge->battery_data->cv_mixCap);
		reinit_status += max77705_write_and_verify_word(fuelgauge, CVHALFTIME_REG,
			fuelgauge->battery_data->cv_HalfTime);
		reinit_status += max77705_write_and_verify_word(fuelgauge, CONVGCFG_REG,
			fuelgauge->battery_data->convgCfg);
		reinit_status += max77705_write_and_verify_word(fuelgauge, 0x60, 0x0000);

		/* 8. Initiate Model Loading */
		Config2value = max77705_read_word(fuelgauge->i2c, CONFIG2_REG); //read Config2(0xBB)
		if (Config2value >= 0)
			reinit_status += max77705_write_and_verify_word(fuelgauge, CONFIG2_REG,
				((Config2value) | (0x0020))); // Setting the LdMdl bit in the Config2 register

		/* 9. Verify Model Loading Completion, normall 400ms */
		for (i = 0; i < 40; i++) {
			Config2value = max77705_read_word(fuelgauge->i2c, CONFIG2_REG);
			if ((Config2value & 0x20) == 0)
				break;
			msleep(10);
		}
		...
		/* To configure SOC matching to OCV for battery removal case */
		if (read_val & 0x8800 && read_val >= 0)
			reinit_status += max77705_write_and_verify_word(fuelgauge, STATUS_REG,
				(read_val & 0x77FF));

		/* 10. Initialization Complete!! Soft POR */
		/* Read Status */
		read_val = max77705_read_word(fuelgauge->i2c, STATUS_REG);

		/* If POR is set during initialization, then do Initialization Again!!*/
		if (read_val & 0x02) 
			fail = 3;
		else if (reinit_status < 0)/* If I2C write fails!!*/
			fail = 4;
		else /* If POR=0, then Initialization Complete!!*/
			fail = 0;
		} while (0);

	pr_info("%s : Finished!! (type:%s, fail:%d)\n", __func__,
		fuelgauge->battery_data->type_str, fail);

	if (!fail) {
		fuelgauge->reinit_needed = false;
		fuelgauge->event_on_por = true;
		/*Write correct FG data version*/
		max77705_write_and_verify_word(fuelgauge, TALRTTH2_REG,
			fuelgauge->data_ver);
		pr_info("%s : data_ver (%d)\n", __func__, fuelgauge->data_ver);
	}
	pr_info("%s : Re-init needed? (%d); event: (%d)\n",
		__func__, fuelgauge->reinit_needed, fuelgauge->event_on_por);
	...
	max77705_fg_test_read(fuelgauge);
...
static void set_reinit_needed(struct max77705_fuelgauge_data *fuelgauge, int status) {

	fuelgauge->reinit_needed = true;
	if(fuelgauge->event_on_por) {
#if IS_ENABLED(CONFIG_SEC_ABC) && !defined(CONFIG_SEC_FACTORY)
		sec_abc_send_event("MODULE=battery@WARN=fg_por");
#endif
		fuelgauge->event_on_por = false;
	}
	pr_info("%s: POR occurred, status=(0x%x)\n", __func__, status);
...
{
	int fg_soc = 0, fg_vcell, avg_current, status;
	...
	status = max77705_read_word(fuelgauge->i2c, STATUS_REG);
	if (fuelgauge->info.soc != 0 && (status < 0)) { //i2c error, return previous soc
		pr_info("%s: i2c fail , return prev SOC(%d), status=(0x%x)\n", __func__,
			fuelgauge->info.soc, status);
		return fuelgauge->info.soc;
	} else if (fuelgauge->info.soc != 0 && (status & 0x02)) {
		set_reinit_needed(fuelgauge, status);
		pr_info("%s: return SOC(%d)\n", __func__, fuelgauge->info.soc);
		return fuelgauge->info.soc;
	}
...
static int max77705_lost_soc_get(struct max77705_fuelgauge_data *fuelgauge, int request_soc)
{
	...
	int report_soc, status;
	if (fuelgauge->reinit_needed) {
		return request_soc;
	}

	/* get current values */
	raw_soc = max77705_get_fuelgauge_value(fuelgauge, FG_RAW_SOC);
	remcap = max77705_get_fuelgauge_value(fuelgauge, FG_REPCAP);
	qh = max77705_get_fuelgauge_value(fuelgauge, FG_QH);

	status = max77705_read_word(fuelgauge->i2c, STATUS_REG); //check POR
	if ((status < 0) || (raw_soc == -1) || (remcap == -1) || (qh == -1)) {
		pr_info("%s: i2c fail or POR, status=(0x%x), return soc(%d)\n", __func__,
			status, request_soc);
		return request_soc;
	} else if (status & 0x02) {
		set_reinit_needed(fuelgauge, status);
		pr_info("%s: return SOC(%d)\n", __func__, request_soc);
		return request_soc;
	}
...
static int reinit_status(struct max77705_fuelgauge_data *fuelgauge) {
	u16 status = 0;

	status = max77705_read_word(fuelgauge->i2c, STATUS_REG);
	if (status & 0x02) {
		set_reinit_needed(fuelgauge, status);
		pr_info("%s: quickstart & initialize FG\n", __func__);
		/* do quickstart */
		if (max77705_fg_reset(fuelgauge)) {
			/* Initialize FG */
			if (max77705_fg_initialization(fuelgauge)) {
				pr_info("%s: FG Initialization FAIL!!\n", __func__);
				return -1;
			}
			else {
...
static int reinit_needed_soc(struct max77705_fuelgauge_data *fuelgauge) {
	fuelgauge->capacity_old = (fuelgauge->capacity_old <= 0) ? 0 : fuelgauge->capacity_old - 1;
	pr_info("%s: POR already occurred reducing the capacity by 1 new cap:%d \n", __func__, fuelgauge->capacity_old);
	return fuelgauge->capacity_old;
}
...
static int max77705_get_fg_ui_soc(struct max77705_fuelgauge_data *fuelgauge, union power_supply_propval *val)
{
	...
	if (val->intval == SEC_FUELGAUGE_CAPACITY_TYPE_RAW) {
		...
		current_time = ktime_get_boottime_seconds();
		if (!fuelgauge->soc_update_time) {
			fuelgauge->soc_update_time = current_time;
			pr_info("%s: soc update time(%lld)\n",
				__func__, (long long)fuelgauge->soc_update_time);
		} else {
			if (!fuelgauge->sleep_initial_update_of_soc && !fuelgauge->initial_update_of_soc) {
				if ((current_time - fuelgauge->soc_update_time) < 10) {
					pr_info("%s: return old capacity (%d) soc_update_last(%lld), soc_update_current(%lld)\n",
						__func__, fuelgauge->capacity_old, (long long)fuelgauge->soc_update_time, (long long)current_time);
					return fuelgauge->capacity_old;
				}
			}
		}
		...
		if (reinit_status(fuelgauge) < 0) {
			return reinit_needed_soc(fuelgauge);
		}
...
static int max77705_fuelgauge_parse_dt(struct max77705_fuelgauge_data *fuelgauge)
{
	...
	if (np == NULL) {
		...
		if (np == NULL) {
			...
			snprintf(prop_name, PROPERTY_NAME_SIZE, "battery%d,%s",
				battery_id, "fullcaprep");
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->fullcaprep);
			if (ret < 0)
				pr_err("%s: error reading fullcaprep %d\n", __func__, ret);

			snprintf(prop_name, PROPERTY_NAME_SIZE, "battery%d,%s",
				battery_id, "iavg_empty");
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->iavg_empty);
			if (ret < 0)
				pr_err("%s: error reading iavg_empty %d\n", __func__, ret);
			...
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->learnCFG);
			if (ret < 0)
				pr_err("%s: error reading learnCFG %d\n", __func__, ret);

			snprintf(prop_name, PROPERTY_NAME_SIZE, "battery%d,%s",
				battery_id, "atRate");
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->atRate);
			if (ret < 0)
				pr_err("%s: error reading atRate %d\n", __func__, ret);
			...
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->config);
			if (ret < 0)
				pr_err("%s: error reading config %d\n", __func__, ret);
			...
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->config2);
			if (ret < 0)
				pr_err("%s: error reading config2 %d\n", __func__, ret);

			snprintf(prop_name, PROPERTY_NAME_SIZE, "battery%d,%s",
				battery_id, "fullsocthr");
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->fullsocthr);
			if (ret < 0)
				pr_err("%s: error reading fullSOCthr %d\n", __func__, ret);
...
/*
			snprintf(prop_name, PROPERTY_NAME_SIZE, "battery%d,%s",
				battery_id, "taskPeriod");
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->taskPeriod);
			if (ret < 0)
				pr_err("%s: error reading taskPeriod %d\n", __func__, ret);
*/
			snprintf(prop_name, PROPERTY_NAME_SIZE, "battery%d,%s",
				battery_id, "tempNom");
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->tempNom);
			if (ret < 0)
				pr_err("%s: error reading tempNom %d\n", __func__, ret);
			...
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->tempLim);
			if (ret < 0)
				pr_err("%s: error reading tempLim %d\n", __func__, ret);

			snprintf(prop_name, PROPERTY_NAME_SIZE, "battery%d,%s",
				battery_id, "cv_mixCap");
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->cv_mixCap);
			if (ret < 0)
				pr_err("%s: error reading cv_mixCap %d\n", __func__, ret);

			snprintf(prop_name, PROPERTY_NAME_SIZE, "battery%d,%s",
				battery_id, "cv_HalfTime");
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->cv_HalfTime);
			if (ret < 0)
				pr_err("%s: error reading cv_HalfTime %d\n", __func__, ret);
/*
			snprintf(prop_name, PROPERTY_NAME_SIZE, "battery%d,%s",
				battery_id, "smartChgCfg");
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->smartChgCfg);
			if (ret < 0)
				pr_err("%s: error reading smartChgCfg %d\n", __func__, ret);
*/
			snprintf(prop_name, PROPERTY_NAME_SIZE, "battery%d,%s",
				battery_id, "convgCfg");
			ret = of_property_read_u32(np, prop_name,
				&fuelgauge->battery_data->convgCfg);
			if (ret < 0)
				pr_err("%s: error reading convgCfg %d\n", __func__, ret);

			for (i = 0; i < CHAR_TABLE_MAX; i++) {
				snprintf(prop_name, PROPERTY_NAME_SIZE, "battery%d,%s%d",
						battery_id, "cell_character", i);

				ret = of_property_read_u32_array(np, prop_name, char_table, 16);
				if (ret < 0)
					pr_err("%s: Can't get prop %s (%d)\n", __func__, prop_name, ret);

				for (j = 0; j <= CHAR_TABLE_LEN; j++)
					fuelgauge->battery_data->cell_character[i][j] = char_table[j];
			}
			...
			if (p) {
				...
				if (ret) {
					...
					for (i = 0; i < fuelgauge->verify_selected_reg_length; i++) {
						...
						else if (fuelgauge->verify_selected_reg[i].addr == ICHGTERM_REG)
							fuelgauge->battery_data->ichgterm = fuelgauge->verify_selected_reg[i].data;
						else if (fuelgauge->verify_selected_reg[i].addr == RELAXCFG_REG)
							fuelgauge->battery_data->relaxcfg = fuelgauge->verify_selected_reg[i].data;
						else if (fuelgauge->verify_selected_reg[i].addr == TGAIN_REG)
							fuelgauge->battery_data->tgain = fuelgauge->verify_selected_reg[i].data;
						else if (fuelgauge->verify_selected_reg[i].addr == TOFF_REG)
							fuelgauge->battery_data->toff = fuelgauge->verify_selected_reg[i].data;
						else if (fuelgauge->verify_selected_reg[i].addr == CGAIN_REG) {
							fuelgauge->battery_data->cgain_origin = fuelgauge->verify_selected_reg[i].data;
							fuelgauge->battery_data->cgain_charging = fuelgauge->verify_selected_reg[i].data;
						} else if (fuelgauge->verify_selected_reg[i].addr == COFFSET_REG) {
							fuelgauge->battery_data->coff_origin = fuelgauge->verify_selected_reg[i].data;
							fuelgauge->battery_data->coff_charging = fuelgauge->verify_selected_reg[i].data;
						}
					}
				}
			} else {
